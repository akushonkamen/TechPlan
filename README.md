...
| 软件 | 用途 | 安装方式 |
|-----|-----|---------|
| SQLite | 调试 SQLite 数据库（应用会自动创建数据库文件） | `sqlite3` CLI |
...
Kuzu 图数据库缓存也会在首次启动时自动创建，文件位于项目根目录 `database.kuzu`。
...
**A:** 图谱接口会在 Kuzu 缺少主题数据时触发后台同步，并先返回 SQLite fallback 数据。可以手动触发同步：
...
- **数据库**: SQLite + Kuzu 本地图缓存
- **AI**: OpenAI / Google Gemini / 自定义兼容 API
- **可视化**: 自定义 SVG 知识图谱 + Recharts
