...
- **判断标准**：该信号是否可能改变竞争格局/技术路线/市场预期？是 → 纳入；否 → 归入 `dataHighlights`
...
2. **快速验证**：查 events/claims 是否有佐证
   - 有佐证 → 纳入 `keyUpdates`，significance = "高"
   - 无佐证 → 纳入 `alerts`（anomaly 类型），标注需持续观察
3. **更新频率**：同一假设如果在 3 个日报中连续出现但无新证据 → 降级为常规监测
