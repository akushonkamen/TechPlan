...
- 不要描述、分析或提及该时间范围之外的事件、趋势或变化（环比对比数据除外）
- 数据查询已经按时间范围过滤，你只需要分析返回的数据
- 标题、摘要和正文中不要假设具体的天数（如"90天"、"一个季度"），使用实际日期范围即可
- 环比/对比分析中引用的上期数据仅作为对比参照，主体分析不得扩展到当前时间范围之外

---

## 第一阶段：数据收集

使用 Bash 工具执行以下 Node.js 脚本查询指定时间范围数据 + 历史对比：
