...
| **AI** | Codex CLI with stream-json output |
...
│   ├── skillExecutor.ts  # Codex CLI execution engine
...
TechPlan spawns Codex CLI with `--output-format stream-json` to get structured progress:

```typescript
const claudeCmd = `Codex -p ${shellEscape(config.prompt)} --output-format stream-json --verbose`;
